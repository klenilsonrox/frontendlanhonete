    ...
    <div className='bg-red-600 text-white py-2 relative'>
      <button className='absolute right-4 top-5 border p-1 text-white  lg:hidden z-20 flex items-center gap-2' onClick={abrirMenu}>Menu <span className={`hamb ${openMenu ? "ativo":""}`}></span></button>
        <header className="p-4 max-w-7xl mx-auto flex justify-between items-center flex-col lg:flex-row relative">
            ...
            <ul className={`flex items-center  flex-col text-gray-900 bg-white lg:bg-red-600 lg:text-white lg:flex-row transition-all ${openMenu ? "h-screen":"h-0"} overflow-hidden lg:h-auto absolute w-full top-[60px]  lg:pt-0 lg:w-auto lg:top-0 lg:static`}>
                ...
                {perfilOpen && <div className='lg:absolute top-[55px] font-medium  bg-white text-gray-600 shadow-sm border w-[200px] p-4 mt-2 animaPerfil flex flex-col '>
                  <Link href="/conta/perfil" onClick={closeMenu} className='border-b-transparent hover:border-b py-1 hover:border-b-green-600 relative flex items-center gap-3 before:h-2 before:w-2 before:bg-green-600 before:rounded-full before:flex'>ver perfil</Link>
                  {user.isAdmin && <Link href="/conta/dashboard" onClick={closeMenu} className='border-b-transparent hover:border-b py-1 hover:border-b-green-600 relative flex items-center gap-3 before:h-2 before:w-2 before:bg-green-600 before:rounded-full before:flex'>Admin</Link>}
                  <li onClick={logout} className='border-b-transparent hover:border-b hover:border-b-red-600 relative flex py-1 items-center gap-3 before:h-2 before:w-2 before:bg-red-600 before:rounded-full before:flex'>Sair</li>
                </div>}
                </li>}
